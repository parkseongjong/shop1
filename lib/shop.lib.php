...
// 베리몰 상품 선택옵션
function get_item_options_barryMall($it_id, $subject,$table, $is_div='', $is_first_option_title='')
{
    global $g5;
    ...
    $sql = " select * from {$g5['g5_shop_item_option_table']} where io_type = '0' and it_id = '$it_id' and io_me_table = '$table' and io_use = '1' order by io_no asc ";
    ...
    $str = '';
    $subj = explode(',', $subject);//상품에 저장된 옵션 이름 데이터
    $subj_count = count($subj);
    ...
    if($subj_count > 1) {
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            for($k=0; $k<$subj_count; $k++) {
                ...
                if(!is_array($ioTypes[$k])){
                    $ioTypes[$k] = array();
                }

                if($opt_id[$k] && !in_array($opt_id[$k], $options[$k])){
                    $options[$k][] = $opt_id[$k];
                    $ioTypes[$k][] = $row['io_type'];
                }
        ...
        for($i=0; $i<$subj_count; $i++) {
            ...
            if($opt_count) {
                ...
                for($k=0; $k<$opt_count; $k++) {
                    $opt_val = $opt[$k];
                    if(strlen($opt_val)) {
                        $select .= '<option value="'.$opt_val.'"  data-option-title="'.$subj[$i].'" data-option-value="'.$opt_val.'" data-io-type="'.$ioTypes[$i][$k].'">'.$opt_val.'</option>'.PHP_EOL;
                    }
                }
                $select .= '</select>'.PHP_EOL;
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            //coin
                ...
                if($row['io_price_etp3'] >= 0){
                    $etp3 = '&nbsp;&nbsp;+ '.number_format($row['io_price_etp3']).'eTP3';
                }
                else{
                    $etp3 = '&nbsp;&nbsp; '.number_format($row['io_price_etp3']).'eTP3';
                }
                if($row['io_price_emc'] >= 0){
                    $emc = '&nbsp;&nbsp;+ '.number_format($row['io_price_emc']).'eMC';
                }
                else{
                    $emc = '&nbsp;&nbsp; '.number_format($row['io_price_emc']).'eMC';
                }
            ...
            $select .= '<option value="'.$row['io_id'].','.$row['io_price'].','.$row['io_price_etp3'].','.$row['io_price_emc'].','.$row['io_stock_qty'].'" data-option-title="'.$subj[0].'" data-option-value="'.$row['io_id'].'" data-io-type="'.$row['io_type'].'">'.$row['io_id'].$price.$soldout.'</option>'.PHP_EOL;
        }
        $select .= '</select>'.PHP_EOL;
