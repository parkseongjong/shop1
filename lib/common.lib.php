...
function get_file($bo_table, $wr_id)
{
    ...
    //대표 사진 count, 상품 정보 사진 count
    $file['count'] =  $file['itemDetailcount'] = 0;
    ...
    while ($row = sql_fetch_array($result))
    {
        ...
        if($row['bf_storage'] == 'itemDetail'){
            $file['itemDetailcount']++;
        }
        else{
            $file['count']++;
        }
...
{
    return get_text(cut_str($subject, $len, $suffix),1);
}
...
// 회원 정보를 얻는다. (barry cyberTron member id 값으로.)
function get_member_mb2($mb_2, $fields='*', $is_cache=false)
{
    global $g5;

    $mb_2 = preg_replace("/[^0-9a-z_]+/i", "", $mb_2);

    static $cache = array();

    $key = md5($fields);

    if( $is_cache && isset($cache[$mb_2]) && isset($cache[$mb_2][$key]) ){
        return $cache[$mb_2][$key];
    }

    $sql = " select $fields from {$g5['member_table']} where mb_2 = TRIM('$mb_2') ";

    $cache[$mb_2][$key] = run_replace('get_member', sql_fetch($sql), $mb_2, $fields, $is_cache);
