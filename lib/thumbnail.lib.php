...
function get_list_multi_thumbnail($bo_table, $wr_id, $thumb_width, $thumb_height, $is_create=false, $is_crop=true, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3')
{
    global $g5, $config;
    $filename = $alt = "";
    $edt = false;
    ...
    if($row['bf_file']) {
        ...
        for($i=0; $i<count($matches[1]); $i++)
        {
            ...
            $p = parse_url($matches[1][$i]);
            if(strpos($p['path'], '/'.G5_DATA_DIR.'/') != 0)
                array_push($data_path,preg_replace('/^\/.*\/'.G5_DATA_DIR.'/', '/'.G5_DATA_DIR, $p['path']));
            else
                array_push($data_path, $p['path']);

            $srcfile = G5_PATH.$data_path[$i];
            ...
            if(preg_match("/\.({$config['cf_image_extension']})$/i", $srcfile) && is_file($srcfile)) {
                ...
                array_push($filename,basename($srcfile));
                array_push($filepath,dirname($srcfile));

                preg_match("/alt=[\"\']?([^\"\']*)[\"\']?/", $matches[0][$i], $malt);
                array_push($alt,get_text($malt[1]));

                //break; 1개가 아닌 다중 처리 합니다.
            }
        }
    }
    if(!$filename[0]){
    ...
    for($j=0; $j<count($filename);$j++){
        $tname = thumbnail($filename[$j], $filepath[$j], $filepath[$j], $thumb_width, $thumb_height, $is_create, $is_crop, $crop_mode, $is_sharpen, $um_value);
        if($tname) {
            if($edt) {
                $src = G5_URL.str_replace($filename[$j], $tname, $data_path[$j]);
            } else {
                $src = G5_DATA_URL.'/file/'.$bo_table.'/'.$tname;
            }
        } 
        ...
        $thumbRerult[$j]['src'] = $src;
        $thumbRerult[$j]['alt'] = $alt;
    }
