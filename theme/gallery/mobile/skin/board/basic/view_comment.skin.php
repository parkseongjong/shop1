...
<!-- 댓글 리스트 -->
<section id="bo_vc">
    <h2><button type="button" class="cmt_btn">댓글목록 <span><?php echo number_format($view['wr_comment']) ?></span></button></h2>
    <?php
    $cmt_amt = count($list);
    for ($i=0; $i<$cmt_amt; $i++) {
    ...
    <article id="c_<?php echo $comment_id ?>" <?php if ($cmt_depth) { ?>style="margin-left:<?php echo $cmt_depth ?>px;border-top-color:#e0e0e0"<?php } ?>>
        <header>
            <h3><?php echo get_text($list[$i]['wr_name']); ?>님의 댓글<?php if ($cmt_depth) { ?><span class="sound_only">의 댓글</span><?php } ?></h3>
        ...
        <div class="cmt_contents">
            ...
            ?>
                <ul class="bo_vc_act">
                    <?php if ($list[$i]['is_reply']) { ?><li class="btn_rpl"><a href="<?php echo $c_reply_href; ?>" onclick="comment_box('<?php echo $comment_id ?>', 'c'); return false;">답변</a></li><?php } ?>
                    <?php if ($list[$i]['is_edit']) { ?><li><a href="<?php echo $c_edit_href; ?>" onclick="comment_box('<?php echo $comment_id ?>', 'cu'); return false;"><i class="fa fa-edit"></i><span class="sound_only">수정</span></a></li><?php } ?>
                    <?php if ($list[$i]['is_del'])  { ?><li><a href="<?php echo $list[$i]['del_link']; ?>" onclick="return comment_delete();"><i class="fa fa-trash-o"></i><span class="sound_only">삭제</span></a></li><?php } ?>
                </ul>
            <?php } ?>
        </div>
        ...
        <div class="bo_vc_w_wr">
            ...
            <div class="btn_confirm">
                <span class="bo_vc_secret">
                    ...
                    <label for="wr_secret" class="icon_lock">비밀글사용</label>
                </span>
