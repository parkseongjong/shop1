...
?>
<script src="http://barrybarries.kr/js/bpopup.js"></script>
<header id="hd">
...
<!--    <?php if ($is_admin) { ?><div class="admin_bar"><a href="<?php echo G5_ADMIN_URL ?>"> 관리자</a></div> <?php } ?>-->
    ...
    <div id="hd_wrapper">
        ...
        <div id="logo">
            <a href="<?php echo G5_URL ?>"><img src="<?php echo G5_THEME_URL ?>/img/logo_3.png" alt="<?php echo $config['cf_title']; ?>"></a>
        </div>
<?php/*
        <div id="logo">
            <a href="<?php echo G5_URL ?>"><?=$config['cf_title'] ?></a>
        </div>
*/?>
        ...
        <button type="button" id="setting_go"><span class="sound_only"> 설정</span></button>

        <div id="top_cate">
            <ul>
               <!-- <li class="<?=($top_cate=='1')?'on':''?>" onclick="goShopping()">쇼핑</li>
                <li class="<?=($top_cate=='2')?'on':''?>" onclick="goCar()">자동차</li>
                <li class="<?=($top_cate=='3')?'on':''?>" onclick="goEstate()">부동산</li>
                <li class="<?=($top_cate=='4')?'on':''?>" onclick="goMarket()">매장광고</li>
                <li class="<?=($top_cate=='5')?'on':''?>" onclick="goUsed()">중고</li>
                -->
                <li class="<?=($top_cate=='1')?'on':''?>" onclick="goShopping()">P2P쇼핑</li>
                <li class="<?=($top_cate=='2')?'on':''?>" onclick="goCar()">자동차</li>
                <li class="<?=($top_cate=='3')?'on':''?>" onclick="goEstate()">부동산</li>
                <li class="<?=($top_cate=='4')?'on':''?>" onclick="goMarket()">벼룩시장광고</li>
                <!-- <li class="<?=($top_cate=='5')?'on':''?>" onclick="goUsed()">중고</li>-->
            </ul>
        </div>
	  ...
	  $('#xx1').bPopup().close();
        ...
        $(function () {
            ...
            $("#setting_go").on("click", function() {
                document.location.href = "/bbs/member_setting.php";
            });
