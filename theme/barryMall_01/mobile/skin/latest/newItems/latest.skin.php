...
<article class="newItemsLatest">
    ...
    <?php for ($i=0; $i<$list_count; $i++) :
        ...
        if($list[$i]['wr_price_type'] == "KRW" ) {
            ...
            $price = "<p>".number_format($list[$i]['wr_3'])."<span class='unit'>e-KRW</span></p>";
        }
        else if($list[$i]['wr_price_type']== "ECTC"){
            $price = "<p>".number_format($list[$i]['wr_4'])."<span class='unit'>e-CTC</span></p>";
        }
...
?>
    ...
    <li class="empty">상품이 없습니다.</li>
    <?php endif; ?>
    </ul>
    <a href="<?php echo get_pretty_url($bo_table); ?>" class="more">
        <button type="button" class="btn btn-secondary btn-block"><?php echo $bo_subject ?> 더 보기</button>
    </a>
</article>
