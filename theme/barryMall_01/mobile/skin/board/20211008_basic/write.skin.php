    ...
    <div class="form-group">
        ...
        <article class="coinTypeCheckArea mb-3">
            ...
            <ul class="list-group coinType">
                ...
                <li class="list-group-item" data-type="ekrw">e-KRW 코인으로 판매<b>(e-KRW)</b></li>
                <li class="list-group-item" data-type="ectc">e-CTC 코인으로 판매<b>(e-CTC)</b></li>
                <li class="list-group-item" data-type="krw">현금으로 판매<b>(원)</b></li>
                <?php if(in_array($member['mb_id'],BARRY_CORP_ARRAY) ||$member['mb_id'] == '0260911133' ): ?>
                <li class="list-group-item" data-type="card">카드로 판매<b>(원)</b></li>
                <?php endif; ?>
            </ul>
        </article>

<!-- 환산중 -->
        <article id="moneySelect">
            <section class="krwCostingArea">
                ...
                <div id="krwCosingWait" class="flex-column align-items-center alert alert-info mb-1">
                    <strong>현금(원) <i class="fa fa-arrow-right"></i>환산 중</strong>
                    <div class="spinner-border text-success" role="status">
                        <span class="sr-only">변환 중</span>
                    </div>
                </div>
                <div class="flex-column align-items-center alert alert-info mb-1">
                    <strong id="coinPerValue"></strong> <strong id="coinUnit"> </strong>당 <strong id="coinRate"></strong>원
                </div>
            </section>
        </article>
<!-- 환산중 -->
    ...
    <article>
        <section>
            <div class="input-group mb-1">
                ...
                <input type="number" name="minimumRetailPrice" value="<?php echo (isset($write['it_minimum_retail_price']))?$write['it_minimum_retail_price']:0 ?>" id="minimumRetailPrice" class="form-control" size="50" min="1" >
            </div>
        </section>
    </article>
    <!-- 쇼핑몰 최저가 -->
    ...
    <!-- e-tp3,e-mc 현금(원)-->
        ...
        <!--       ekrw 현금(원)-->
        <article id="ekrwSelect">
            <h3 class="badge badge-dark">- 현금(원)을 입력하시면 e-KRW 판매 금액이 자동 적용 됩니다.</h3>
            <h3 class="badge badge-dark">- e-KRW 입력 하시면 코인 결제가능합니다.</h3>
            <section class="ekrwArea">
                <div class="input-group mb-1">
                    <div class="input-group-prepend">
                        <span class="input-group-text">e-KRW</span>
                    </div>
                    <input type="number" name="priceEkrw" value="<?php echo ($wr_3==0)?'':$wr_3 ?>" id="priceEkrw"  class="form-control" size="30" min="1" placeholder="현금(원) → e-KRW 판매금액" required readonly>
                </div>
            </section>
        </article>
        <!--       ekrw 현금(원)-->
        ...
        <!--      ectc 현금(원)-->
        <article id="ectcSelect">
            <h3 class="badge badge-dark">- 현금(원)을 입력하시면 e-CTC 판매 금액이 자동 적용 됩니다.</h3>
            <h3 class="badge badge-dark">- e-CTC 입력 하시면 코인 결제가능합니다.</h3>
            <section class="ectcArea">
                <div class="input-group mb-1">
                    <div class="input-group-prepend">
                        <span class="input-group-text">e-CTC</span>
                    </div>
                    <input type="number" name="priceEctc" value="<?php echo ($wr_4==0)?'':$wr_4 ?>" id="priceEctc"  class="form-control" size="30" min="1" placeholder="현금(원) → e-CTC 판매금액" required readonly>
                </div>
            </section>
        </article>
        <!--       ectc 현금(원)-->
    ...
    <?php } ?>
        ...
        <div class="itemContents <?php echo $is_dhtml_editor ? $config['cf_editor'] : ''; ?>">
            ...
            <div class="custom-file mt-1 mb-1">
                <input type="file" id="imageSearchItemDetail" name="imageSourceItemDetail[]" class="custom-file-input" accept="media_type, .jpg, .jpeg, .gif, .png" multiple="multiple" required>
                <label class="custom-file-label" for="imageSearchItemDetail">상품 정보 사진 선택 하기</label>
            </div>
            <ul id="imageSearchItemDetailList">

            </ul>
        </div>
    </div>

        <!-- 2021.09.13 상품 선택 옵션 비활성화 -->
        <!--
    <div class="form-group">
        <div class="title">
            <h3>상품 선택옵션(선택 항목)</h3>
