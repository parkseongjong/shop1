...
$(function($){
    ...
    $(document).on("change", "select.it_option", function() {
        ...
        if(barryView.cartPriceType == 'KRW'){
            //최종 선택된 옵션 value는 선택옵션 value,현금가,e-TP3가,e-MC,e-KRW,e-CTC가 입니다.
            ...
            else if(barryView.cartPriceType == 'e-KRW'){
                cartOptionPrice = cartOptionPriceArray[4];
            }
            else if(barryView.cartPriceType == 'e-CTC'){
                cartOptionPrice = cartOptionPriceArray[5];
            }
        }
    ...
    function initBirthday() {
        barryView.cardPaymentAuth = true;
        $('#btnBirthday').addClass('on');
        $('.companyNumberArea').hide();
        $('.birthDayArea').show();

        barryView.cardPaymentAuthType = 'birthDay';
        $('input[name="companyNumber"]').attr('disabled',true);
        $('select[name="birthDayYear"]').attr('disabled',false);
        $('select[name="birthDayMonth"]').attr('disabled',false);
        $('select[name="birthDayDay"]').attr('disabled',false);

        $('.birthDayArea').show();
        $('#cardDataTitle').text('생년월일');
    }

    function initCompanyNumber() {
        barryView.cardPaymentAuth = true;
        $('.companyNumberArea').show();
        $('.birthDayArea').hide();

        barryView.cardPaymentAuthType = 'companyNumber';
        $('input[name="companyNumber"]').attr('disabled',false);
        $('select[name="birthDayYear"]').attr('disabled',true);
        $('select[name="birthDayMonth"]').attr('disabled',true);
        $('select[name="birthDayDay"]').attr('disabled',true);

        $('#cardDataTitle').text('사업자등록번호');
    }


    // 카드결제 생년월일/사업자등록번호 등록
    $(document).on("click",'.paymentAuthArea > .btn', function(){
        $('.paymentAuthArea > .btn').removeClass('on');
        $(this).addClass('on');

        if($(this).data('type') == 'brithday'){
            /* iOS에서 데이트 픽커 문제 발생. year와 month가 선택이 안됨
            brithdayEl.attr('placeholder','900103').attr('maxlength','6').datetimepicker({
        ...
        else if($(this).data('type') == 'companyNumber'){
            initCompanyNumber();
        }
        else{
            barryView.cardPaymentAuth = false;
            bsCommonAlert('입력 하실 정보를 선택 해주세요,','danger');
        }
    ...
    $(document).on("submit",'#orderUpload',async function(){
        ...
        barryView.memberAddress = f.memberZip.value+' '+f.memberAddr1.value+' '+f.memberAddr2.value+' '+f.memberAddr3.value+' '+f.memberJibun.value;
        ...
        if(paymemntRequrie == true && barryView.priceType != 'KRW'){
            ...
            if(barryView.cartPriceType == 'CREDITCARD'){
                ...
                unit = barryView.cartPriceType;
            }

            $.confirm({
                title: '구매확인',
                content: barryView.sellerName+'('+barryView.sellerId+')님에게 '+amount+' '+unit+'를 결제 하시겠습니까?',
                buttons: {
                    cancel:{
                        text: '취소',
...
$(document).on("submit",'#paymentCardform',async function(){
    ...
    //생년월일 또는 사업자등록 번호 선택을 안한 경우 오류
    if(barryView.cardPaymentAuth == false){
        bsCommonAlert('입력 하실 정보를 선택 해주세요,','danger');
        return false;
    }
    ...
    let orderUploadFormData = new FormData($(this)[0]);
    //생년월일 또는 사업자 등록 번호 폼... 데이터 확인

    if(barryView.cardPaymentAuthType == 'birthDay'){
        if(checkSpace(orderUploadFormData.get('birthDayYear'))){
            bsCommonAlert('생년월일을 선택 해주세요.','danger');
            return false;
        }
        if(checkSpace(orderUploadFormData.get('birthDayMonth'))){
            bsCommonAlert('생년월일을 선택 해주세요.','danger');
            return false;
        }
        if(checkSpace(orderUploadFormData.get('birthDayDay'))){
            bsCommonAlert('생년월일을 선택 해주세요.','danger');
            return false;
        }
    }
    else if(barryView.cardPaymentAuthType == 'companyNumber'){
        if(checkSpace(orderUploadFormData.get('companyNumber'))){
            bsCommonAlert('사업자 등록 번호를 입력해주세요.','danger');
            return false;
        }
    }
    else{
        bsCommonAlert('생년월일 또는 사업자 등록 번호를 입력해주세요.,','danger');
        return false;
    }
    ...
    $.ajax({
        ...
        error:function (request, status, error){
            bsCommonAlert('결제가 실패 되었습니다.(사용자 네트워크가 불안정 합니다.)', 'danger');
            setTimeout(function() {
                btnDisabledStatus(submitBtnTarget,false);
            },3000);
        }
    ...
    $.ajax({
        ...
        success : function(data, textStatus) {
            ...
            if(data.code == 200){
                ...
                if(paymemntRequrie == true && barryView.priceType == 'TP3MC' || barryView.priceType == 'TP3' || barryView.priceType == 'MC' || barryView.priceType == 'EKRW' || barryView.priceType == 'ECTC'){
                    $('#paymentLayer').modal('show');//일반 코인 결제
                    //카드결제 일 때 검증용 어마운트 값 삽입..!
                    $('input[name=amount]').val(barryView.previewItemTotalPrice);

                }
                else if(paymemntRequrie == true && barryView.priceType == 'CREDITCARD' && barryView.cartPriceType == 'CREDITCARD'){
...
function checkFrontValid(theForm,barryView){
    ...
    else if(checkSpace(theForm.memberZip.value)){
        bsCommonAlert('주문자 배송지 주소가 비어 있습니다. 주소 검색을 해주세요.');
        theForm.memberZip.focus();
        return false;
    }
    else if(checkSpace(theForm.memberAddr1.value)){
        bsCommonAlert('주문자 배송지 주소가 비어 있습니다. 주소 검색을 해주세요.');
        theForm.memberAddr1.focus();
        return false;
    }
    else if(checkSpace(theForm.memberAddr2.value)){
        bsCommonAlert('주문자 배송지 상세 주소를 입력해주세요.');
        theForm.memberAddr2.focus();
        return false;
    }
    else if(checkSpace(barryView.memberAddress)){
        bsCommonAlert('주문자 배송지 주소가 비어 있습니다. 확인해주세요.');
        return false;
    }
