        ...
        <ul class="btn_bo_user2">
            <li>
                <select name="price_type" id="price_type" class="custom-select">
                    ...
                    <option value="EKRW" <?php if($price_type =="EKRW"){ echo 'selected';}?>>e-KRW</option>
                    <option value="ECTC" <?php if($price_type =="ECTC"){ echo 'selected';}?>>e-CTC</option>
...
//        
        ...
        if($list[$i]['wr_price_type'] == "KRW" ) {
            $price = "<span>".number_format($list[$i]['wr_10'])."<span class='unit'>원</span></span>";
        }
        else if($list[$i]['wr_price_type']== "TP3MC") {
            $price ='';
            if($list[$i]['wr_1'] != 0){
                $price .= "<span>".number_format($list[$i]['wr_1'])."<span class='unit'>e-TP3</span></span>";
            }
            if($list[$i]['wr_2'] != 0){
                $price .= "<span>".number_format($list[$i]['wr_2'])."<span class='unit'> e-MC</span></span>";
            }

        }
        else if($list[$i]['wr_price_type']== "EKRW"){
            $price = "<span>".number_format($list[$i]['wr_3'])."<span class='unit'>e-KRW</span></span>";
        }
        else if($list[$i]['wr_price_type']== "ECTC"){
            $price = "<span>".number_format($list[$i]['wr_4'])."<span class='unit'>e-CTC</span></span>";
        }
        ...
        else if ($list[$i]['wr_price_type'] == "CREDITCARD"){
            $price = "<span>".number_format($list[$i]['wr_10'])."<span class='unit'>원</span></span>";
        }
 ?>
                    ...
                    <div class="card-body">
                        ...
                        <a href="<?=$list[$i]['href'] ?>" class="listSbjA">
                            ...
                            <div class="card-text">
                                <div class="amount">
                                    <?php echo $price ?>
                                    <?php if($list[$i]['it_minimum_retail_price'] > 0 ): ?>
                                        <?php if ($bo_table == 'offline' && $list[$i]['wr_price_type'] == "KRW" || $list[$i]['wr_price_type'] == "CREDITCARD"): ?>
                                            <?php
                                                // 상품금액 - 최저가
                                                $tempPrice = $list[$i]['it_minimum_retail_price'] - $list[$i]['wr_10'];
                                                //할인율
                                                $bulidMinRetailPricePercent = ($tempPrice/$list[$i]['it_minimum_retail_price'])*100;
                                            ?>
                                            <strike class="minimum">
                                                <?php echo number_format($list[$i]['it_minimum_retail_price']) ?> 원
                                            </strike>
                                            <?php if($bulidMinRetailPricePercent >= 40): ?>
                                                <span class="discountRate">
                                                    <?php printf('%02d',$bulidMinRetailPricePercent) ?>%
                                                </span>
                                            <?php endif; ?>
                                        <?php endif; ?>
                                    <?php endif; ?>
                            ...
                            <div>
                                <?php if($list[$i]['wr_price_type'] == "CREDITCARD"): ?>
                                    <span class="badge badge-dark">카드/현금 결제</span>
                                <?php elseif($list[$i]['wr_price_type'] == "KRW"): ?>
