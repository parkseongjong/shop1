...
//기존에 BBS UPDATE 코어는 상품 관련 수정이 많아, 미디어 게시판 게시글 업로드랑 분리합니다.
    if($board['bo_1'] == 'BBS') {
        $delete_href = G5_BBS_URL . '/deleteRealBbs.php?bo_table=' . $bo_table . '&amp;wr_id=' . $wr_id . '&amp;token=' . $token . '&amp;page=' . $page . urldecode($qstr);
    }
    else{
...
*/
 ...
 //wr_id 값이 있는경우 it_id 값에 넣어주기
$is_orderable = true;
if($is_orderable) {
    if(defined('G5_THEME_USE_OPTIONS_TRTD') && G5_THEME_USE_OPTIONS_TRTD){
        $option_item = get_item_options_barryMall($view['wr_id'], $view['it_option_subject'],$view['it_me_table'], '');
        $supply_item = get_item_supply($view['wr_id'], $view['it_supply_subject'], '');
    } else {
        // 선택 옵션 ( 기존의 tr td 태그로 가져오려면 'div' 를 '' 로 바꾸거나 또는 지워주세요 )
        $option_item = get_item_options_barryMall($view['wr_id'], $view['it_option_subject'],$view['it_me_table'], 'div', 1);
        // 추가 옵션 ( 기존의 tr td 태그로 가져오려면 'div' 를 '' 로 바꾸거나 또는 지워주세요 )
        $supply_item = get_item_supply($view['wr_id'], $view['it_supply_subject'], 'div', 1);
    }

    // 상품 선택옵션 수
    $option_count = 0;
    if($view['it_option_subject']) {
        $temp = explode(',', $view['it_option_subject']);
        $option_count = count($temp);
    }

    // 상품 추가옵션 수
    $supply_count = 0;
    if($view['it_supply_subject']) {
        $temp = explode(',', $view['it_supply_subject']);
...
//include_once($board_skin_path.'/view.skin.20200522.php');
