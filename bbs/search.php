...
if ($stx) {
    ...
    $srows = isset($_GET['srows']) ? (int)preg_replace('#[^0-9]#', '', $_GET['srows']) : 20;
    if (!$srows) $srows = 20; // 한페이지에 출력하는 검색 행수
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        ...
        $g5_search['tables'][] = $row['bo_table']; //자동차 p2p 벼룩시장 .
    ...
    for ($i=0; $i<count($g5_search['tables']); $i++){
        ...
        $sql = " select wr_id from {$tmp_write_table} where {$sql_search} and del_yn='N' and (it_publish = 1 OR it_publish = 99)";
...
';
    ...
    if ($onetable)
    {
    $g5_search['tables'] = Array();// 하나의 게시판만 검색한다
        $sql = "  select gr_id, bo_table, bo_read_level from {$g5['board_table']} where bo_use_search = 1 and bo_list_level <= '{$member['mb_level']}'  and bo_table = '{$onetable}'";
    ...
    if($gr_id) {
    for ($idx=$table_index; $idx<count($g5_search['tables']); $idx++) {
        $sql = " select bo_subject, bo_mobile_subject from {$g5['board_table']} where bo_table = '$gr_id' ";
        $row = sql_fetch($sql);
        $bo_subject[$idx] = ((G5_IS_MOBILE && $row['bo_mobile_subject']) ? $row['bo_mobile_subject'] : $row['bo_subject']);


        
        $tmp_write_table = $g5['write_prefix'] . $g5_search['tables'][$idx];

        $sql = " select * from {$tmp_write_table} where {$sql_search} and del_yn='N' and it_me_table ='$gr_id' and (it_publish = 1 OR it_publish = 99) order by wr_datetime desc limit {$from_record}, {$rows}  ";
        $result = sql_query($sql);
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            $list[$idx][$i] = $row;
            $list[$idx][$i]['href'] = get_pretty_url($g5_search['tables'][$idx], $row['wr_parent']);
  ...
  for ($idx=$table_index; $idx<count($g5_search['tables']); $idx++) {
      $sql = " select bo_subject, bo_mobile_subject from {$g5['board_table']} where bo_table = '{$g5_search['tables'][$idx]}' ";
      $row = sql_fetch($sql);
      $bo_subject[$idx] = ((G5_IS_MOBILE && $row['bo_mobile_subject']) ? $row['bo_mobile_subject'] : $row['bo_subject']);

      $tmp_write_table = $g5['write_prefix'] . $g5_search['tables'][$idx];

      $sql = " select * from {$tmp_write_table} where {$sql_search} and del_yn='N' and (it_publish = 1 OR it_publish = 99) order by wr_datetime desc limit {$from_record}, {$rows}  ";
      $result = sql_query($sql);
...
$group_select = '<label for="gr_id" class="sound_only">게시판 그룹선택</label><select name="gr_id" id="gr_id" class="custom-select"><option value="">전체 분류';
$sql = " select gr_id, gr_subject from {$g5['group_table']} order by gr_order ";
$result = sql_query($sql);
for ($i=0; $row=sql_fetch_array($result); $i++)
