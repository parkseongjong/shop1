...
$editor_html = editor_html('itemContents', $content, $is_dhtml_editor);
$editor_js = '';
$editor_js .= get_editor_js('itemContents', $is_dhtml_editor);
$editor_js .= chk_editor_js('itemContents', $is_dhtml_editor);
...
//기존에 BBS UPDATE 코어는 상품 관련 수정이 많아, 미디어 게시판 게시글 업로드랑 분리합니다.
if($board['bo_1'] == 'BBS'){
    $action_url = https_url(G5_BBS_DIR)."/write_updateRealBbs.php";
}
else{
...
if($write['it_publish'] == 1){
    $goodsInfo = $goodsStatusInfo = array();
    //status 가 1 인 경우 긍정에 가까운 결과 입니다.
    $goodsStatusInfo['goodsId'] = array('title'=>'상품 번호','type'=>'text', 'status' => 1, 'msg'=>$write['wr_id']);
    $goodsStatusInfo['publish'] = array('title'=>'승인 여부');
    $goodsStatusInfo['deleteStatus'] = array('title'=>'상품 판매 상태');
    $goodsStatusInfo['soldout'] = array('title'=>'상품 품절 여부');
    $goodsStatusInfo['category'] = array('title'=>'상품 카테고리', 'type'=>'text', 'status' => 1, 'msg'=>$write['ca_name']);
    $goodsStatusInfo['datetime'] = array('title'=>'상품 등록 일시', 'type'=>'text', 'status' => 1, 'msg'=>$write['wr_datetime']);

    array_push($goodsStatusInfo['publish'],array('status'=>1,'msg'=>'승인'));
    $goodsStatusInfo['publish']['status'] = 1;
    $goodsStatusInfo['publish']['msg'] = '승인';
    if ($write['del_yn']=='Y'){
        $goodsStatusInfo['deleteStatus']['status'] = 0;
        $goodsStatusInfo['deleteStatus']['msg'] = '판매중지(삭제됨)';
    }
    else{
        $goodsStatusInfo['deleteStatus']['status'] = 1;
        $goodsStatusInfo['deleteStatus']['msg'] = '판매중(정상)';
    }
    if($write['it_soldout']==0){
        $goodsStatusInfo['soldout']['status'] = 1;
        $goodsStatusInfo['soldout']['msg'] = '정상';
    }
    else{
        $goodsStatusInfo['soldout']['status'] = 0;
        $goodsStatusInfo['soldout']['msg'] = '품절';
    }
    $goodsInfo['thumb'] = get_list_thumbnail($write['it_me_table'], $write['wr_id'], 150, 150, false, true);
    if($goodsInfo['thumb'] === false){
        $goodsInfo['thumb']['src'] = false;
    }
    $goodsInfo['content'] = utf8_strcut(strip_tags(conv_content($write['wr_content'],1)),50);
    include_once ($board_skin_path.'/writeModify.skin.php');
}
else{
