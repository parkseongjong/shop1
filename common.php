...
** 공통 변수, 상수, 코드
*******************************************************************************/
//cafe24 rewrite가 무조건 host 네임을 리다이렉션 하기 때문에 php 단에서 처리 합니다.
if(!isset($_SERVER['HTTPS']) && $_SERVER['HTTPS']!='on'){
	header('Location: https://barrybarries.kr'.$_SERVER['REQUEST_URI']);
}
...
if (isset($_REQUEST['price_type']))  { // 검색 조건 추가, 201015, YMJ
    if ($price_type)
        $qstr .= '&amp;price_type=' . urlencode($price_type);
} else {
    $price_type = '';
}
...
$gbCoupon = new gbCoupon();
//$gbCoupon->sellerCheck();
//$gbCoupon->premiumCheck();
if($_SERVER['REMOTE_ADDR'] == '211.44.188.4'){
	//$gbCoupon->adItemInsert();
	//$gbCoupon->adItemCheck();
	//$gbCoupon->adItemUpdate();
}
//$gbCoupon->adItemCheck();
//$gbCoupon->adItemReport();
//unset($gbCoupon);
//latest.lib 에서 global로 사용 합니다. 변수를 언셋하지 않습니다.
//BARRY AUTH 확인 END
...
//request content-type이 application/json 일때 response도..  
