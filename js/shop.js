    ...
    */
    ...
    $(document).on("change", "select.it_option", function() {
        var sel_count = $("select.it_option").size(),
            ...
            val = $(this).val(),
            it_id = barryView.itemId,
            //post_url = (typeof g5_shop_url !== "undefined") ? g5_shop_url+"/itemoption.php" : g5_board_skin_url+"/option/user/itemOption.php",
            $this = $(this),
            op_0_title = $this.find("option:eq(0)").text();

        let post_url = g5_url+"/API/barry/goods/item/option/select/"+g5_bo_table+"/"+it_id;
        console.log(idx);
        ...
        if(sel_count > 1 && (idx + 1) < sel_count) {
            ...
            $.get(
                post_url,
                //{ it_id: it_id, opt_id: opt_id, idx: idx, sel_count: sel_count, op_title : op_0_title, bo_table : g5_bo_table },
                { optId: opt_id, totalCount: sel_count, thisCallIndex: idx, thisCallTitle: op_0_title, type:'single'},
                function(data) {
                    $("select.it_option").eq(idx+1).empty().html(data.html).attr("disabled", false);
            ...
            //베리베리 몰에서는 input value에 항목이 추가 되어 변경 됩니다.
            if(parseInt(info[4]) < 1) {
