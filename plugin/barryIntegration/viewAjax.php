...
try {
    ...
    $targetBo_table_array = array('Shop','car','estate','market','testtestbarry','offline');
    ...
    if(in_array($target_bo_table,$targetBo_table_array)) {
        switch ($target_bo_table) {
            ...
            case 'market':
                ...
                break;
            case 'testtestbarry':
                $boTableFilter = 'testtestbarry';
                break;
            case 'offline':
                $boTableFilter = 'offline';
                break;
        }
    }
    ...
    $ret = array();
    $dbObject = $db->prepare('
        SELECT A.wr_id, A.wr_datetime, A.wr_1, A.wr_2, A.wr_3, A.wr_4, A.wr_5, A.wr_6, A.wr_7, A.wr_8, A.wr_10, A.wr_11, A.wr_12, A.wr_status, A.wr_price_type, A.wr_ct_price, A.ct_option, A.wr_io_price, A.od_number, B.it_soldout, B.mb_id, B.wr_subject, B.wr_content, B.wr_seo_title, B.del_yn
    ...
    if($dbObject->rowCount() == 0){
        array_push($ret,jsonResponseCommon(404));
            ...
            $dbDriver = $db->prepare('
                ...
                WHERE bpps_order_id = ? AND bpps_status = "complete"
            ');
            $dbDriver->bindValue(1, $row['wr_id'], PDO::PARAM_STR);
            $dbDriver->execute();
            $orderAuthNumber = $dbDriver->fetch();
            if(empty($orderAuthNumber['bpps_auth_number'])){
                $orderAuthNumber['bpps_auth_number'] = false;
            }
            if(!$orderAuthNumber){
                $orderAuthNumber = false;
            }
            $cardNameJsonType = json_decode($orderAuthNumber['bpps_response'],true);
            //var_dump($cardNameJsonType);
            ...
            array_push($ret,
                       array(
                            ...
                            'itemOrderAuthNumber'=>$orderAuthNumber['bpps_auth_number'],
                            'itemOrderNumber'=>$row['od_number'],
                            'cardResponse' =>$cardNameJsonType['cardName']
                            ));
        }
